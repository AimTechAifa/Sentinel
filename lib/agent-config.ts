...
export const AGENT_GRADIENTS: Record<AgentRole, string> = {
  "Ticket Agent": "from-violet-500 via-purple-500 to-fuchsia-500",
  ...
  "Summary Agent": "from-indigo-500 via-violet-500 to-purple-500",
...
export function getAgentGradient(agent: AgentMeta): string {
  return agent.accent || AGENT_GRADIENTS[agent.name] || "from-violet-500 to-brand-500";
}
