...
export function ChatPanel() {
  ...
  return (
    <>
      <button onClick={toggle} className="fixed bottom-6 right-6 w-14 h-14 bg-ai text-white rounded-full shadow-lg flex items-center justify-center hover:bg-violet-700 transition-colors z-50">
        {open ? <X className="w-6 h-6" /> : <MessageCircle className="w-6 h-6" />}
      </button>
      {open && (
        <div className="fixed top-0 right-0 z-40 flex h-full w-96 flex-col border-l border-gray-200 bg-white shadow-theme-md">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm shadow-theme-sm ${m.role === "user" ? "bg-brand-500 text-white" : "bg-violet-50 border border-violet-100 text-gray-800"}`}>
